...
export default function ActiveContactsPage() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      const res = await fetch(`/api/outbound/contacts?locationId=${locId}&tag=ai_enabled`);
      const data = await res.json();
      setContacts(data.contacts || []);
      setAiCount(data.aiEnabledCount || 0);
    } catch (e) {
    ...
    <div style={{ padding: "clamp(20px,3vw,36px)", maxWidth: "1200px" }}>
      ...
      <div style={{ marginBottom: "24px" }}>
        ...
        <p style={{ color: "#2d3d50", margin: 0, fontSize: "13px" }}>
          Activate your <code style={{ background: "rgba(20,184,166,0.1)", color: "#14b8a6", padding: "1px 6px", borderRadius: "4px" }}>contacts</code> to automatically handle them to your AI sales team.
        </p>
      </div>
