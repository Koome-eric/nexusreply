...
export default function AnalyticsPage() {
  ...
  const [tab, setTab] = useState<"ai" | "sales" | "agents">("ai");
  const busy = useRef(false);

  useEffect(() => {
    if (busy.current || !locationId) return;
    busy.current = true;
    setLoading(true);
    const p = new URLSearchParams({ days: String(days), locationId });
  ...
  return (
    <div style={{ padding: "clamp(20px,3vw,36px)", maxWidth: "1180px" }}>
      ...
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", flexWrap: "wrap", gap: "12px", marginBottom: "28px" }}>
        <div>
          <h1 style={{ fontSize: "clamp(20px,3.5vw,26px)", fontWeight: 800, color: "#e2eaf4", margin: "0 0 4px", letterSpacing: "-0.03em" }}>Analytics</h1>
          <p style={{ color: "#2d3d50", fontSize: "13px", margin: 0 }}>Real data from your AI sales team. No vanity metrics.</p>
        </div>
      ...
      <div style={{ display: "flex", gap: "4px", marginBottom: "24px", background: "rgba(10,17,30,0.6)", borderRadius: "10px", padding: "4px", border: "1px solid rgba(255,255,255,0.06)", width: "fit-content" }}>
        {([{ key: "ai", label: "🤖 AI Performance" }, { key: "sales", label: "📊 Sales Pipeline" }, { key: "agents", label: "🏆 Agent Comparison" }] as const).map(t => (
            ...
            <>
              ...
              <div style={{ background: "rgba(167,139,250,0.05)", border: "1px solid rgba(167,139,250,0.12)", borderRadius: "12px", padding: "14px 18px" }}>
                ...
                <div style={{ fontSize: "12px", color: "#445566", lineHeight: 1.7 }}>
                  <strong style={{ color: "#7c9ab8" }}>Low autonomy</strong> = agent lacks confidence on common topics — tighten its system prompt. <strong style={{ color: "#7c9ab8" }}>Low win rate</strong> = closing logic needs strengthening. Go to <a href="/dashboard/agents" style={{ color: "#14b8a6", textDecoration: "none" }}>AI Sales Team</a> to edit prompts, or ask your admin to update the global template.
                </div>
              </div>
            </>
          )}
