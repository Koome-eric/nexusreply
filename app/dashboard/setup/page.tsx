...
interface Location { id: string; name: string; ghlLocationId: string }

function SetupContent() {
  const params = useSearchParams();
  const preselectedId = params.get("locationId");
  ...
  useEffect(() => {
    fetch("/api/locations").then(r => r.json()).then(d => {
      ...
      setLocations(locs);
      const pick = preselectedId || locs[0]?.id || "";
  ...
  useEffect(() => {
    ...
    setLoading(true);
    fetch(`/api/business?locationId=${selectedLocationId}`).then(r => r.json()).then(d => {
    ...
    return (
      <div style={{ padding: "clamp(20px,4vw,40px)", maxWidth: "600px" }}>
        <div className="glass" style={{ borderRadius: "16px", padding: "48px", textAlign: "center" }}>
          ...
          <h2 style={{ fontSize: "20px", fontWeight: 700, marginBottom: "10px" }}>No locations connected</h2>
          <p style={{ color: "var(--text-secondary)", fontSize: "14px", marginBottom: "24px" }}>Connect a GoHighLevel account first, then come back to train your AI.</p>
          <a href="/dashboard/locations"><button className="btn-primary">Connect GoHighLevel →</button></a>
        </div>
      </div>
    );
  }
    ...
    <div style={{ padding: "clamp(20px,4vw,40px)", maxWidth: "860px" }}>
      ...
      {locations.length > 1 && (
        <div className="fade-in-up" style={{ marginBottom: "24px" }}>
          <label style={{ display: "block", fontSize: "12px", fontWeight: 700, color: "var(--text-secondary)", marginBottom: "8px", letterSpacing: "0.04em" }}>CONFIGURING LOCATION</label>
          <select className="input-field" value={selectedLocationId} onChange={e => setSelectedLocationId(e.target.value)} style={{ maxWidth: "380px" }}>
