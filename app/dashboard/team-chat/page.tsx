...
export default function TeamChatPage() {
  ...
  useEffect(() => {
    ...
    fetch(`/api/agents?locationId=${selectedLoc}`).then(r => r.json()).then(d => setAgents(d.agents || []));
  }, [selectedLoc]);

  const loadMessages = useCallback(() => {
    if (!selectedLoc) return;
    const p = new URLSearchParams({ locationId: selectedLoc });
  ...
  const send = async () => {
    ...
    const res = await fetch("/api/team-chat", {
      ...
      body: JSON.stringify({ message: txt, locationId: selectedLoc, targetAgent: targetAgent || undefined }),
    });
          ...
          <div style={{ display: "flex", gap: "8px", alignItems: "center", flexWrap: "wrap" }}>
            {locations.length > 1 && (
              <select value={selectedLoc} onChange={e => setSelectedLoc(e.target.value)}
                style={{ padding: "6px 10px", borderRadius: "8px", background: "rgba(10,17,30,0.9)", border: "1px solid rgba(255,255,255,0.08)", color: "#7c9ab8", fontSize: "11px", fontFamily: "inherit", cursor: "pointer" }}>
